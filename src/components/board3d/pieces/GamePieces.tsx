...
export function RoadPiece({
  ax, az, bx, bz, y, color, ghost = false, onClick,
}: {
  ax: number; az: number; bx: number; bz: number; y: number; color: string; ghost?: boolean; onClick?: PieceClick;
}) {
  const dx = bx - ax;
  const dz = bz - az;
  ...
  return (
    <group position={[(ax + bx) / 2, y, (az + bz) / 2]} rotation={[0, -angle, 0]} onClick={onClick}>
      <mesh position={[0, .025, 0]} castShadow>
        <boxGeometry args={[len, .09, .16]} />
        <meshStandardMaterial color={color} roughness={.72} metalness={.04} transparent={ghost} opacity={ghost ? .55 : 1} emissive={ghost ? "#f59e0b" : "#000"} emissiveIntensity={ghost ? .35 : 0} />
      </mesh>
      {!ghost && <mesh position={[0, .078, 0]}>
        <boxGeometry args={[len * .92, .018, .075]} />
        <meshStandardMaterial color="#F6F2E7" roughness={.95} opacity={.34} transparent />
      </mesh>}
    </group>
  );
}
  ...
  return (
    <group position={[x, y, z]} onClick={onClick}>
      <mesh position={[0, .05, 0]} rotation={[-Math.PI/2,0,0]}>
        <circleGeometry args={[.29, 24]} />
        <meshBasicMaterial color="#111" transparent opacity={.22} depthWrite={false} />
      </mesh>
...
function ClassicSettlement({ color }: { color:string }) {
  return <>
    ...
    <mesh position={[.02,.15,.145]}><boxGeometry args={[.07,.12,.018]}/><Stone color="#F6F2E7"/></mesh>
  </>;
}

function MedinaSettlement({ color }: { color:string }) {
  return <>
...
function LimestoneSettlement({ color }: { color:string }) {
  return <>
...
export function KnightPiece({ x, z, y, color, level = 1, active = false, theme, onClick }: { x:number; z:number; y:number; color:string; level?:number; active?:boolean; theme:Theme; onClick?:PieceClick }) {
  ...
  const muted = active ? color : "#756f66";
  ...
  return <group position={[x,y+.02,z]} scale={scale} rotation={[0,pose,0]} onClick={onClick}>
    ...
    <mesh position={[0,.19,0]} castShadow><cylinderGeometry args={[.10,.13,.30,8]}/><meshStandardMaterial color={muted} roughness={.72}/></mesh>
    ...
    <group position={[.12,.24,.08]} rotation={[0,0,shieldRot]}>
      <mesh castShadow><cylinderGeometry args={[.13,.13,.035,${8}]}/><meshStandardMaterial color="#F6F2E7" roughness={.75}/></mesh>
      {Array.from({length:rank}).map((_,i)=><mesh key={i} position={[-.045+i*.045,.019,0]}><boxGeometry args={[.018,.016,.08]}/><meshStandardMaterial color={color} roughness={.55}/></mesh>)}
    </group>
